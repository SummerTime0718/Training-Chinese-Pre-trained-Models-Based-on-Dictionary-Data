...
def train_vocab(vocab_size):
    ...
    prefix = "./data/tokenizer/tok" + str(vocab_size)
    ...
    #把中文辭典資料檔案轉換成作者丟入sentencePiece工具的格式
    data_file = './dict_set.txt'
    # 2) train the sentencepiece model
    print("Will now train the vocab... and use %s"%(data_file))
...
#for txt file
def txt_shard(file_name,vocab_size):
    print("txt_shard running now")
    if vocab_size == 0:
        print("Using llama tokenizer now")
    else:
        print("Using custom tokenizer ,vocab_size = ",vocab_size)
    shard = file_name
    tokenizer_model = get_tokenizer_model_path(vocab_size)
    enc = Tokenizer(tokenizer_model)
    ...
    with open(shard, "r", encoding='utf-8') as f:
        data = f.readline()
        while data:
            tokens = enc.encode(data.strip(), bos=True, eos=False)
            all_tokens.extend(tokens)
            data = f.readline()
    ...
    if vocab_size == 0:
        ...
        # save .bin files into a new tok{N} directory
        tokenized_filename = './data/tokenizer/tok13304.bin'
    with open(tokenized_filename, "wb") as f:
        f.write(all_tokens.tobytes())
    # calculate the average sequence length (they are separated by BOS=1)
    avg_seq_len = all_tokens.size / ((all_tokens == 1).sum())
...
def pretokenize(vocab_size):
    ...
    shard_filename = "./data/allData2025.txt"
    if vocab_size > 0:
...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        if self.vocab_source == "llama2":
            ...
            shard_filenames = './data/allData2025.bin'#m1125503
